...
const submit = async () => {
  try {
    await router.post('/admin/courses/', form);
  } catch (error) {
...
<template>
  <Layout>
    <div class="flex items-center justify-between">
      <h1 class="mb-4 text-2xl font-semibold">Création d'une matière</h1>
    </div>
    <div>
      <div class="mb-2" v-for="(_, field) in form">
        ...
        <select
          ...
          multiple
        >
          <option disabled selected>Choisissez une unité d'enseignement</option>
          <option v-for="unit in fieldValues[field]" :key="unit.id" :value="unit.id">
            {{ unit.name }}
          </option>
        </select>
